...
function mod:PallunAI(npc, sprite, d)
    ...
    if d.state == "hiding" then
        if npc.StateFrame > 60 then
            d.state = "idle"
            npc.StateFrame = 0
            npc.Position = mod:freeGrid(npc, false, 250, 50)
            mod:spritePlay(sprite, "Appear")
    ...
    if npc.StateFrame > 20 and d.state ~= "shooting" and d.state ~= "hiding" and game:GetRoom():CheckLine(target.Position,npc.Position,3,900,false,false) and not d.shotAlive then
        d.state = "shooting"
        mod:spritePlay(sprite, "Shoot")
    elseif npc.StateFrame > 20 and d.state ~= "shooting" and d.state ~= "hiding"  then
        d.state = "hiding"
        mod:spritePlay(sprite, "Leave")
    elseif npc.StateFrame > 20 and d.state ~= "hiding" then
        npc.StateFrame = 0
    end
    ...
    if sprite:IsEventTriggered("Shoot") then
        ...
        d.p:GetData().offset = math.random(0, 40) 
    ...
    function mod.PallunShot(v, d)
        if d.type == "Pallun" then
            if not d.Parent:IsDead() then
                ...
                v.Height = -20
                v.Velocity = mod:Lerp(npc.Velocity, (d.targpos - v.Position) + (d.targpos - v.Position):Normalized()*1.05 + d.myrand1 , 100/1000)
