...
mod:AddCallback(ModCallbacks.MC_USE_PILL, function(_, pill, player, flags)
    if flags ~= flags | UseFlag.USE_OWNED and flags ~= flags | UseFlag.USE_ALLOWNONMAIN and flags ~= flags | UseFlag.USE_MIMIC and mod:AnyPlayerHasCollectible(mod.Collectibles.Items.Tums) then
...
mod:AddCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, function(_, pickup, coll, bool)
    if not mod:AnyPlayerHasCollectible(mod.Collectibles.Items.Tums) then return end
