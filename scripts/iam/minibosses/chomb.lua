...
mod:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, function(_, npc, offset)
    if npc.Variant == mod.MiniBosses.Chomb.Var then
        mod:ChombRenderAI(npc, npc:GetSprite(), npc:GetData())
    end
end, mod.MiniBosses.Chomb.ID)
...
local function GetChombDist(table, element)
...
local function FindEntsWithSameInitSeedChomb(npc)
    local tab = {}
    for k, butt in ipairs(Isaac.FindByType(mod.MiniBosses.Chomb.ID, mod.MiniBosses.Chomb.Var)) do
...
local function GetDipsWithSameDataChomb(npc)
    local tab = {}
...
function mod:ChombAI(npc, sprite, d)
    ...
    local function HasPossibleParentSegs(butts) --if a butt has any parents with the same data for "initseed" (not the actual thing)
        ...
        if d.butts and #d.butts > 0 then
            ...
            if not butts:IsDead() and GetChombDist(tab, d.butts[1].Position) and tab2[GetChombDist(tab, d.butts[1].Position)]:GetData().IsSegment == false then
                return tab2[GetChombDist(tab, d.butts[1].Position)]
            end
        end
    end
    ...
    if d.IsSegment then
        ...
        local mvstr = mod:GetMoveString(npc.Velocity, true, true)
        ...
        if d.state == "Moving" then
            ...
            elseif npc.StateFrame < d.SegNumber*2 and npc.Position:Distance(d.OldPos) == 0 then
                npc:SetSpriteFrame(d.name .. d.animExtraName ..mod:GetMoveString(npc.Velocity, true, false).. d.animExtraName2, 1)
                d.state = npc.Parent:GetData().state
            end
        else
        ...
        if d.IsButt and sprite:IsEventTriggered("Poop") and #GetDipsWithSameDataChomb(npc) <= 3 then
    ...
    elseif not d.IsSegment then
        ...
        if #FindEntsWithSameInitSeedChomb(npc) then
            for k, v in ipairs(FindEntsWithSameInitSeedChomb(npc)) do
                if v:GetData().IsSegment == false then
                    d.IsSegment = true
                    table.insert(FindEntsWithSameInitSeedChomb(npc)[1]:GetData().butts, npc)
                    d.SegNumber = #FindEntsWithSameInitSeedChomb(npc)[1]:GetData().butts+1
                    return
                end
            end
        end
        ...
        d.SegNumber = 1
        d.MovementAnim =mod:GetMoveString(npc.Velocity, true, true)
        ...
        mod:spritePlay(sprite, d.name .. d.animExtraName ..mod:GetMoveString(npc.Velocity, true, true) .. d.animExtraName2, true)

        if sprite:IsFinished(d.name .. "Strain" ..mod:GetMoveString(npc.Velocity, true, true)) then
...
function mod:ChombRenderAI(npc, sprite, d)
    ...
    local function HasPossibleParentSegs(butts) --if a butt has any parents with the same data for "initseed" (not the actual thing)
        ...
        if d.butts and #d.butts > 0 then
            ...
            if not butts:IsDead() and GetChombDist(tab, d.butts[1].Position) and GetChombDist(tab, d.butts[1].Position) > 0 then
                return tab2[GetChombDist(tab, d.butts[1].Position)]
            end
        end
    end
...
--tha-thanks ff.........
function mod:ChombGetHurt(npc, amount, damageFlags, source)
    ...
    local function HasPossibleParentSegs(butts)
        ...
        if d.butts then
            ...
            if not butts:IsDead() and GetChombDist(tab, d.butts[1].Position) then
                return GetChombDist(tab, d.butts[1].Position)
            end
        end
    end
